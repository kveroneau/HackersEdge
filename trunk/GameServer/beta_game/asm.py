...
OP_CODES = {
    ...
    'BRK': [None, None, None, None, None, None, None, None, None, None, 0x00, None, None],
...
class Assembler(object):
    version = 'HackerASM v0.10.2 $Rev: 197 $'
    ...
    def set_label(self, label, value):
        ...
        if label in self.labels.keys():
            ...
            for ptr in self.labels[label][1]:
                ...
                if label.startswith('@') and self.org > 0:
                    v = value+self.org
                    print hex(v)
        ...
        if op == 'LDA':
            if param.startswith('#>@'):
                if param[2:] in self.hi_var.keys():
                    ...
                    param='#%s' % self.format_hex(self.hi_var[param[2:]][0] >> 8, 2)
            elif param.startswith('#<@'):
                if param[2:] in self.lo_var.keys():
                    self.lo_var[param[2:]][1].append(self.mem.tell())
                    param='#%s' % self.format_hex(self.lo_var[param[2:]][0] & 0xff, 2)
    ...
    def savebin(self, binfile):
        ...
        if self.bintype == 'HEBIN':
            ...
            if self.dmem.tell() > 0:
                raise AssembleError('Data segments cannot be used with this binary type.')
        ...
        with open(binfile, 'wb') as f:
            if self.bintype == 'HEBIN':
                ...
                f.write(chr(0xfe)+chr(self.org >> 8))
