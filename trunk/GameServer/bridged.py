...
from game_engine.bridge import BridgeServer

log = logging.getLogger('Bridged')

def main():
    from optparse import OptionParser
    parser = OptionParser()
    parser.add_option('-l', '--log', dest='logfile', help='Output server log to this file.')
    parser.add_option('-p', '--port', type='int', dest='port', default=4356, help='The port to use for the Bridge server.')
    ...
    if options.daemon:
        ...
        if pid > 0:
            ...
            open('bridged.pid', 'w').write(str(pid))
            sys.exit(0)
        ...
        sys.stdout.write(chr(27)+']2;Bridged'+chr(7))
        sys.stdout.flush()
    ...
    bridged = BridgeServer(('0.0.0.0', options.port))
    log.info('Bridged server started.')
    running = True
    while running:
        try:
